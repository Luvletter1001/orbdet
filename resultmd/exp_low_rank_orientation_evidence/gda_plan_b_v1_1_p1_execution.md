...
| 预算 | ~~≈5.7h~~ **实测 ≈13.2h**（E0 0.168 s/iter → P1 0.735 s/iter；开销主因是探针塔在 5 层 FPN 上的前向+反向，get_targets 基准排除嫌疑：3×N=0.065s 反而快于 1×3N=0.158s。同型号 A40，非硬件差异。优化后路已登记：探针改读 reg_feat 可省两座卷积塔，留给 P2 决策，P1 不中途改架构） |

## 3. 启动保护结果（已执行）

按用户指令跳过独立冒烟复跑、直接长训，前 25 分钟按冒烟标准盯日志：**通过**——无 NaN/inf（精确正则核验）、gda_* 四指标全部在场、loss 13.3→7.3 正常下降、grad_norm 经 clip 35 收敛、显存 9.4–12.3GB/46GB、GPU 利用率 100%、无崩溃。03:18 启动，预计完成 ~17:00–17:30（含逐 epoch val）。
