...
# 追加修正（2026-09-16 12:40，由"为什么在关注点的密度，不该做旁路分支加权吗"这一质疑触发）

本轮之前的分析缺了一个本质分类，导致两处判断偏了。现补上并修正。

## A. 三类操作的本质区分（此前没显式区分，是错误来源）

| 类别 | 操作 | 信息量 | 能否突破 1-D fibre |
| --- | --- | --- | --- |
| **A. 重加权** | 改每个点的权重／归约方式 | **不变** | **不能**（定理直接覆盖） |
| **B. 新增约束** | 加一个把两个**此前未耦合**的量耦合起来的项 | **增加** | **可能** |
| **C. 换信号源** | 换教师／伪标签 | 同维度增加 | 不能（教师给的是同维度更准值，属 A 的范畴） |

**包络欠定结论恰好判掉的正是 A 类**：弱监督 2 DOF vs OBB 3 DOF 的 1-D fibre 说的是"**损失侧任何再加权都无法把质量集中到真角度**"。所以判断一个方案能不能动机制，**只需看它属于哪一类**。

## B. 修正一：`use_reweighted_loss_bbox` 的定位错了
...
- 它在**调制已有项**（用一致性损失的标量去缩放 bbox loss），**不新增任何约束** → **属 A 类**。
- 故按 A 类的性质：**它不能突破 1-D fibre**，只能改变优化动力学。
- **上一轮我称它为"最真实的杠杆／首选"，属定位偏差，现予下调**：它是**优化动力学杠杆**，不是机制杠杆。仍值得跑（零新代码、官方开关），但**不要指望它带来角度定位的机制性改善**。

## C. 修正二："s2 缺跨视图尺寸一致性"应降级

上一轮把它列为"候选空档"。按 A/B/C 分类重新审视：

- box 的**尺寸与中心在弱标签（HBB）里本来就是已知的** → 两个视图各自已有直接监督
- 再把两视图的 box 耦合起来，**在已知量上是冗余的**
- 而**角度在弱标签里未知** → 两视图之间的耦合才**必需**（这正是 `loss_symmetry_ss` 存在、而 box 没有对应项的原因）

→ **s2 缺 `loss_ss_bbox` 极可能是有意省略（消除冗余），不是缺口。** 该候选**降级为"疑似冗余省略"**，不得当作空档使用。

## D. 由此得到 B 类的真实落点

按 B 类定义（把角度与**此前未耦合的量**耦合），s2 上真正空缺的是**信息源**，而非权重：

1. **把角度耦合到图像外观**（边缘／梯度对齐）—— Point2RBox-v2 E2E 用 `EdgeLoss` 做这件事；s2 没有
2. **把角度耦合到实例间布局**（重叠上界／Voronoi 下界）—— Point2RBox-v2 E2E 用 `loss_overlap`(10.0) + `loss_voronoi`(5.0)；s2 没有
3. **合成图案先验** —— s1 用 `basic_pattern` + `use_setsk`；s2 没有

## E. 对 `geometry_aux` 结构的影响（重要，影响可行性）

现有 `AuxBatch` 合同全部是**框中心**的：`student_boxes[N,5]`、`teacher_boxes[N,5]`、`image_indices[N]`、`object_ids[N]`、`angle_valid[N]`、`label_kinds`；`per_item_geometry` 也是逐式的框对框比较（中心按教师尺寸归一、log 尺寸、宽高交换等价框取 min、`cos(2Δθ)`）。

→ 但上面 4.D 的两条落点**恰好有一条能直接塞进现有结构**：

- **布局类约束（重叠上界／Voronoi 下界）本质上就是"每对象算出一个界限框"**（上界框／下界框）→ **可以直接作为第二组 `teacher_boxes` 塞进现有槽位，无需改合同**。这是**成本最低的 B 类落地**。
- **图像外观类（边缘对齐）不是框对框** → **需要给 `AuxBatch` 增加非框信息槽**（如边缘图或梯度量）。这是真实的结构性改动。

## F. 修正后的方向排序

| 优先级 | 方案 | 类别 | 成本 |
| --- | --- | --- | --- |
| 1 | **布局约束作为第二组框塞进现有 core**（重叠上界／Voronoi 下界） | **B** | 低（不改合同） |
| 2 | `use_reweighted_loss_bbox=True` | A | 极低（官方开关），但**只动动力学** |
| 3 | 图像外观耦合（边缘对齐） | **B** | 高（需给 `AuxBatch` 加槽） |
| — | ~~跨视图尺寸一致性~~ | 疑似冗余 | 已降级，不建议 |
| — | `object_mean` 归约 | **A** | 已在跑；是**归约正确性**实验，非机制实验 |

**一句话**：想动机制就必须做 B 类；而 B 类现成的、不用改结构的入口是**"每对象一个界限框"**（重叠／Voronoi）。
