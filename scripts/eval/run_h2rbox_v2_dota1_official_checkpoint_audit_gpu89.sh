...
audit_root=/data1/zcy/Orbdet/work_dirs/audit/h2rbox_v2_dota1_official_20260815
...
on_error() {
  ...
  rtk echo "Official H2RBox-v2 DOTA-v1 audit failed with exit ${exit_code}." >&2
  exit "${exit_code}"
}
trap on_error ERR
...
if rtk pgrep -af \
  '[p]ython.*tools/test.py.*h2rbox_v2_r50_dota1_official' \
  >/dev/null; then
...
if [[ -n "${gpu_processes}" ]]; then
  rtk echo 'GPU 8/9 are not idle; refusing to start official audit.' >&2
  exit 6
fi
...
if [[ -e "${audit_root}/COMPLETE" || \
  ...
  rtk echo 'Existing official-audit outputs found; refusing to overwrite them.' >&2
  exit 7
fi
...
rtk env \
  ...
  "${ss_checkpoint}" \
  --launcher=pytorch \
  --work-dir="${ss_submission_work_dir}"
validate_zip "${ss_zip}"
...
rtk env \
  ...
  "${msrr_checkpoint}" \
  --launcher=pytorch \
  --work-dir="${ms_submission_work_dir}"
validate_zip "${ms_zip}"
rtk touch "${audit_root}/MS_SUBMISSION_COMPLETE"
rtk touch "${audit_root}/COMPLETE"
rtk echo 'Official H2RBox-v2 DOTA-v1 SS/MS checkpoint audit completed.'
