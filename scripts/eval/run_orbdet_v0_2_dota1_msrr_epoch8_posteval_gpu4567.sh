...
stage_root=/data1/zcy/Orbdet/work_dirs/formal/orbdet_v0_2_dota1_ms_rr_gpu4567_seed3407_resume_e3_to_e8_20260818
checkpoint=/data1/zcy/Orbdet/work_dirs/formal/orbdet_v0_2_dota1_ms_rr_gpu4567_seed3407_resume_e3_to_e8_20260818/epoch_8.pth
eval_root=/data1/zcy/Orbdet/work_dirs/eval/orbdet_v0_2_dota1_ms_rr_gpu4567_epoch8_20260818
status_dir="${eval_root}/status"
lock_path=/data1/zcy/Orbdet/work_dirs/.gpu_4_5_6_7.launch_lock
launch_lock_held=0
...
ss_prefix="${ss_work_dir}/orbdet_v0_2_msrr_epoch8_ss_task1"
ms_prefix="${ms_work_dir}/orbdet_v0_2_msrr_epoch8_msrr_task1"
ss_zip="${ss_prefix}/orbdet_v0_2_msrr_epoch8_ss_task1.zip"
ms_zip="${ms_prefix}/orbdet_v0_2_msrr_epoch8_msrr_task1.zip"
...
on_error() {
  ...
  rtk echo "Epoch-8 MS+RR post-evaluation failed with exit ${exit_code}." >&2
  exit "${exit_code}"
}
on_signal() {
  trap - ERR INT TERM
  rtk touch "${status_dir}/INTERRUPTED"
  rtk echo 'Epoch-8 MS+RR post-evaluation reached its absolute deadline.' >&2
  exit 130
}
trap on_error ERR
...
if ! rtk mkdir "${lock_path}"; then
  rtk echo "GPU group 4,5,6,7 launch lock is held: ${lock_path}" >&2
  exit 3
fi
launch_lock_held=1
skip_if_insufficient_window
...
if [[ ! -e "${stage_root}/COMPLETE" || ! -s "${checkpoint}" ]]; then
  rtk echo 'Epoch-8 training is not COMPLETE; refusing post-evaluation.' >&2
  exit 2
fi
...
if [[ -e "${status_dir}/RUNNING" || -e "${status_dir}/COMPLETE" || \
      ...
      -e "${status_dir}/MS_SUBMISSION_COMPLETE" || -e "${trainval_work_dir}" || \
